...
/**
 * Manual test script for US-014 Join Event API
 * 
 * This script tests the join event functionality by:
 * 1. Testing successful join event operation
 * 2. Testing error cases (event not found, already joined, event full, etc.)
 * 
 * Run with: node test-join-event.js
...
const TEST_EVENT_ID_2 = '507f1f77bcf86cd799439012'; // Second test event
const TEST_USER_ID = 'test-user-456'; // Different user for join testing
const TEST_USER_EMAIL = 'join-test@example.com';
...
const tests = [
  {
    ...
    url: `${BASE_URL}/api/events/${TEST_EVENT_ID_2}/join`,
    method: 'POST',
    expectedStatus: 200,
  ...
  },
  {
    ...
    url: `${BASE_URL}/api/events/${TEST_EVENT_ID}/join`,
    method: 'POST',
    userId: 'test-user-123', // This user already joined in leave event tests
    userEmail: 'test@example.com',
    userRole: 'USER',
    expectedStatus: 400,
    description: 'Should return 400 when user already joined the event'
  },
  {
  ...
  {
    name: 'Join Event - Invalid Event ID Format',
    url: `${BASE_URL}/api/events/invalid-id/join`,
    method: 'POST',
    expectedStatus: 404,
    description: 'Should return 404 for invalid event ID format'
  },
  {
    name: 'Join Event - Missing Auth Headers',
    url: `${BASE_URL}/api/events/${TEST_EVENT_ID}/join`,
    method: 'POST',
...
async function runTests() {
  console.log('🧪 Running US-014 Join Event API Tests\n');
  console.log('📋 Test Configuration:');
  console.log(`   Base URL: ${BASE_URL}`);
  console.log(`   Test Event ID: ${TEST_EVENT_ID}`);
  console.log(`   Test Event ID 2: ${TEST_EVENT_ID_2}`);
